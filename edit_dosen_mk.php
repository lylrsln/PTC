...
if (isset($_POST['update'])) {
    ...
    if (empty($dosen_id) || empty($matkul_id) || empty($prodi_id) || empty($jurusan)) {
        ...
        $sql_update = "UPDATE dosen_mk SET dosen_id = ?, matkul_id = ?, prodi_id = ?, jurusan = ?, tahun_akademik_id = ? WHERE dosen_mk_id = ?";
        $stmt_update = $conn->prepare($sql_update);
        $stmt_update->bind_param("iiisii", $dosen_id, $matkul_id, $prodi_id, $jurusan, $tahun_akademik_id, $dosen_mk_id);
