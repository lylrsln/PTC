    ...
    for row in jadwal:
        ...
        jadwal_item = {
            ...
            "waktu": {
                ...
                "hari": row['hari'],
                "jam_mulai": timedelta_to_str(row['jam_mulai']) if isinstance(row['jam_mulai'], timedelta) else row['jam_mulai'].strftime('%Y-%m-%d %H:%M:%S'),
                "jam_selesai": timedelta_to_str(row['jam_selesai']) if isinstance(row['jam_selesai'], timedelta) else row['jam_selesai'].strftime('%Y-%m-%d %H:%M:%S'),
            }
        }
        jadwal_response.append(jadwal_item)
